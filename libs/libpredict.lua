...
libpredict.sender:SetScript("OnEvent", function()
  ...
  elseif strfind(event, "SPELLCAST_START", 1) then
    ...
    if strfind(event, "UNIT_", 1) then -- tbc
      ...
      -- UnitCastingInfo can briefly return nil while the cast event is
      -- already firing (e.g. instant casts, interrupted casts, or client/API timing).
      -- Do not perform arithmetic until both timestamps are available.
