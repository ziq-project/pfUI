pfUI:RegisterModule("addonbuttons", "vanilla:tbc", function ()
  ...
  -- Default third-party minimap icons pfUI can't discover on its own because
  -- they aren't parented to Minimap/MinimapBackdrop (e.g. Project Legacy's
  -- serverside LFG tool, opened via /LFG). Seeded once into the same "add"
  -- list that /abp add writes to, so it's collected for every user without
  -- manual action. On servers where the frame doesn't exist this is a no-op,
  -- since UpdatePanel() only adds names that resolve via _G. If a user
  -- explicitly runs "/abp del <name>" it lands in the "del" list and this
  -- default won't fight that choice on future logins.
  local default_abuttons = { "LFT_Minimap" }
  for _, default_name in ipairs(default_abuttons) do
    local in_add, in_del = false, false
    for _, v in ipairs(pfUI_cache["abuttons"]["add"]) do
      if v == default_name then in_add = true end
    end
    for _, v in ipairs(pfUI_cache["abuttons"]["del"]) do
      if v == default_name then in_del = true end
    end
    if not in_add and not in_del then
      table.insert(pfUI_cache["abuttons"]["add"], default_name)
    end
  end
