pfUI:RegisterModule("chat", "vanilla:tbc", function ()
  ...
  function pfUI.chat:RefreshChat()
    ...
    for i=1, NUM_CHAT_WINDOWS do
      ...
      _G["ChatFrame" .. i .. "TabText"]:SetTextColor((pfUI:GetClassColor(class).r + .3) * .5, (pfUI:GetClassColor(class).g + .3) * .5, (pfUI:GetClassColor(class).b + .3) * .5, 1)
      ...
      for name in gfind(text, "|Hplayer:(.-)|h") do
        ...
        if class then
          if class ~= UNKNOWN then
            color = rgbhex(pfUI:GetClassColor(class))
            match = true
          end
